...
### Cross-Platform Invocation Contract (Use in every assistant)

Use this wrapper sentence before every coding request so behavior stays consistent across OpenCode, Gemini, Warp, and Codex:

> "Use SkillSum skill: `<skill-name>`. Follow `SKILLS-writecode.md` exactly. If project code conflicts with template examples, keep project code style and architecture, then report deviations."

### Required Output Contract for Any Assistant

Every assistant response for implementation work must include:
1. A short plan (2-5 bullets)
2. The exact files changed
3. What was implemented
4. Validation performed (typecheck/test/lint if available)
5. Any follow-up TODOs (if validation could not fully run)

### Safety Contract

- Never invent files, routes, or imports that do not exist in this codebase
- Never replace architecture decisions silently; explain and justify when deviating
- Never skip auth checks, error handling, or type definitions
- Prefer incremental edits over full-file rewrites unless explicitly requested
